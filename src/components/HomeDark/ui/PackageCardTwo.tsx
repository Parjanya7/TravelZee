...
const PackageCardTwo = (
    { id, img, price, title, discountedPrice, disc, destination, currency, handleSubscribe }:
        {
            id: string,
            img: string,
            price: string,
            ...
            <div className="bg-white text-dark-1 px-6 pt-5 pb-base">
                <div className="flex justify-between items-center flex-wrap space-y-2">
                    ...
                    <div className="shrink-0 font-medium text-primary-1 text-[22px] flex items-center ">
                        {!discountedPrice && <span>{currency}{price}</span>}
                        {discountedPrice && <span>{currency}{discountedPrice}</span>}
                        {discountedPrice && <del className="text-sm text-dark-4 ml-[6px]">{currency}{price}</del>}
                    </div>
                </div>
